...
其实就是 MySQL 里经常说到的【WAL技术】，WAL的全称是 **Write-Ahead Logging**，它的关键点就是<u>先写日志，再写磁盘，</u>也就是先写粉板，等不忙的时候再写账本。
...
![img](README.assets/b075250cad8d9f6c791a52b6a600f69c.jpg)



write pos 是当前记录的位置，一边写一边后移，写到第3号文件末尾后就回到 0 号文件开头。checkpoint 是当前要擦除的位置，也是往后推移并且循环的，擦除记录前要把记录更新到数据文件。
...
![img](README.assets/2e5bff4910ec189fe1ee6e2ecc7b4bbe.png)
...
假设当前 `ID = 2` 的行，字段c的值是0。



::: danger 先写 redo log 后写 binlog
